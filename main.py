...
def negate_seq(seq):
    if seq is None:
        return None
    return cp.array([-x for x in seq])
...
class SequencePair:
    ...
    def __eq__(self, other):
        return cp.array_equal(self.a, other.a) and cp.array_equal(self.b, other.b)
        ...
        for key in common_keys:
            ...
            for data_a in list_a:
                for data_b in list_b:
                    ...
                    if cp.array_equal(data_a[0], data_b[0]):
...
def cyclic_autocorrelation(seq):
    ...
    try:
        ...
        if fft is None:
            fft = cp.fft.fft  # Используем CuPy для FFT
            fft_cache[v_len] = fft

        data = cp.zeros(2 * v_len)
        for i in range(v_len):
            data[2 * i] = seq[i]

        fft_data = fft(data)
        data[:v_len] = cp.real(fft_data[:v_len])**2 + cp.imag(fft_data[:v_len])**2
        ...
        result = cp.fft.ifft(data[:v_len]).real
